...
public class HomeActivity extends Activity implements GoalsListAdapter.GoalActionsListener {
    ...
    private static final int PICK_DEPOSIT_REQUEST = 1;
    ...
    public void editGoal(Goal goal) {
        ...
        startActivityForResult(intent, PICK_DEPOSIT_REQUEST);
    }

    @Override
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == PICK_DEPOSIT_REQUEST) {
            if (resultCode == RESULT_OK) {
                if (mSelectedGoal != null) {
                    int depositAmount = data.getIntExtra(EditGoalActivity.KEY_RESULT_AMOUNT, 0);
                    mSelectedGoal.setBalance(mSelectedGoal.getBalance() + depositAmount);
                    if (mGoalsProvider.save(mSelectedGoal)) {
                        mSelectedGoal = null;
                        mGoalsListAdapter.notifyDataSetChanged();
                    } else {
                        Toast.makeText(this, R.string.goal_save_error, Toast.LENGTH_SHORT).show();
                    }
                }
            }
        }
